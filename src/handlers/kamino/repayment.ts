...
export class RepaymentHandler implements TransactionHandler {
  ...
  async build(data: Payload, publicKey: string): Promise<BuildTransactionResponse> {
    ...
    const repaymentAction = await KaminoAction.buildDepositReserveLiquidityTxns(
      market,
      new BN(data.amount),
    ...
    const tx = await buildVersionedTransaction(connection, userPubkey, [
      ...repaymentAction.computeBudgetIxs,
      ...repaymentAction.setupIxs,
      ...repaymentAction.lendingIxs,
      ...repaymentAction.cleanupIxs,
    ]);
