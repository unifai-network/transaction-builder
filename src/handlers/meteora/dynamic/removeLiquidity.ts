...
export class MeteoraDynamicRemoveLiquidityHandler implements TransactionHandler {
...
async function createTransaction(data: Payload, publicKey: PublicKey) {
  ...
  const quote = amm.getWithdrawQuote(amount, data.slippage);

  return await amm.withdraw(new PublicKey(publicKey), quote.poolTokenAmountIn, quote.tokenAOutAmount, quote.tokenBOutAmount);
}
